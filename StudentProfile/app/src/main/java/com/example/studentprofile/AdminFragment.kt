    ...
    fun init(v1: ViewGroup?) {
        ...
        btn_login.setOnClickListener {
            if (id.text != null && pass.text != null) {
                ...
                val file = File(fileDir, "Admin.txt")
                val seperator = "\n"
                if (file.exists()) {
                                ...
                                if (arrayData[j] == pass.text.toString() && j == i + 1) {
                                    ...
                                    if(fileStudent.exists() && fileStudentProfile.exists()) {
                                        ...
                                        if(flg_open == true) {
                                            ...
                                            val intent = Intent(requireContext(), AdminRegistrationForm::class.java)
        ...
        btn_registerHere.setOnClickListener {
            val dialogView = layoutInflater.inflate(R.layout.register_admin, null)
            ...
            btnRegister.setOnClickListener {
                if (newId.text != null && Id.text != null && adminPass.text != null) {
                    ...
                    if (file.exists()) {
                        ...
                        var flg = false
                        for (i in arrayData.indices) {
                            if (arrayData[i].trim() == Id.text.toString()) {
                                var flg2 = false
                                if (Id == newId) {
                                    Toast.makeText(
                                        requireContext(),
                                        newId.text.toString() + " is already an Admin",
                                        Toast.LENGTH_SHORT
                                    )
                                        .show()
                                    flg = true
                                ...
                                if (flg2 == false) {
                                    var flg3 = false
                                    for (j in arrayData.indices) {
                                        if (arrayData[j].trim() == adminPass.text.toString()) {
                                            flg3 = true
                                            val writer = FileWriter(file, true)
                                            writer.write("\n" + newId.text.toString() + "\nAdmin")
                                            writer.close()
            ...
            builder.setTitle("Adding new-Admin")
            builder.setMessage("Please enter data carefully, Once new Admin added he/she will be able to modify the data")
        ...
        btn_reset.setOnClickListener {
            ...
            resetBtn.setOnClickListener {
                if (Id.text != null && newPass.text != null && oldPass.text != null) {
                    ...
                    Id.setError("all fields are required")
                    oldPass.setError("all fields are required")
